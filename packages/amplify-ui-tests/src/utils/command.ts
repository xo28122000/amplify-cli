...
export async function startServer(cwd: string, settings: any) {
  exec('PORT=' + settings.port + ' yarn start', { cwd: cwd });
  //waiting for the server to launch
  await sleep(SEVER_LAUNCH_TIME);
}

export function closeServer(settings: { port: string }) {
  exec(`kill -9 $(lsof -t -i:${settings.port})`);
}

export async function signUpNewUser(cwd: string, settings: any = {}, verbose: boolean = !isCI()) {
  const meta = getProjectMeta(cwd);
  const { UserPoolId, AppClientIDWeb } = Object.keys(meta.auth).map(key => meta.auth[key])[0].output;
  const s = { ...defaultSettings, ...settings, clientId: AppClientIDWeb, userPoolId: UserPoolId };
...
function comfirmSignUp(cwd: string, settings: any, verbose: boolean = !isCI()) {
